...
class CacheBust
{
    ...
    {
        $jsonPath = NOTIFICATIONCENTER_PATH . apply_filters('test/Helper/CacheBust/RevManifestPath', 'dist/rev-manifest.json');

        if (file_exists($jsonPath)) {
            return json_decode(file_get_contents($jsonPath), true);
        } elseif (WP_DEBUG) {
            echo '<div style="color:red">Error: Assets not built. Go to ' . NOTIFICATIONCENTER_PATH . ' and run gulp. See '. NOTIFICATIONCENTER_PATH . 'README.md for more info.</div>';
        }
    }
}
